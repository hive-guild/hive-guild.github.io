...
function renderPublicRoster() {
  ...
  for (const entry of rows) {
    ...
    if (group !== previousGroup) {
      ...
      // Every kind of grouping names its heading with artwork: the role's own for the role sort, the
      // class crest for the class sort, and the spec's icon for the spec sort - the spec line says which
      // class it belongs to, since a spec name alone does not.
      const groupIcon = sortBy === "role" ? roleIcons[group]
        : sortBy === "class" ? classes.find((item) => item.name === group)?.icon
        : specIcons[entry.class_name]?.[group] || classes.find((item) => item.name === entry.class_name)?.icon;
...
function renderAverageRaidDays(entries, prefix = "") {
  ...
  // The script places the panel; the card's :hover shows it, exactly as in the compromise card.
