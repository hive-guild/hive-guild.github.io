...
import { raidRole, raidRoleOrder, raidRoleClass } from "./raid-roles.js";
...
function clearAdminData() {
  state.entries = [];
  for (const selector of ["#stats-grid", "#common-availability", "#most-availability", "#day-chart", "#time-chart", "#roster-list"]) $(selector).replaceChildren();
...
function renderStats() {
  ...
  const definitions = [
    ...
    ["Tanks", state.entries.filter((e) => raidRole(e) === "Tank").length, "#a9a2ff"],
    ["Healer", state.entries.filter((e) => raidRole(e) === "Healer").length, "#8bd9bd"],
    ["Melee DPS", state.entries.filter((e) => raidRole(e) === "Melee DPS").length, "#ff9c9f"],
    ["Ranged DPS", state.entries.filter((e) => raidRole(e) === "Ranged DPS").length, "#c8adff"],
    ["Flexible", state.entries.filter((e) => raidRole(e) === "Flexible").length, "#e3c284"],
  ];
...
function renderAvailabilityInsights(entries, prefix = "") {
  ...
  if (shared.status === "available") {
    ...
    target.append(slots, element("p", "common-caption", `Alle ${shared.count} Rückmeldungen · gemeinsam maximal ${shared.maxDays} Raidtage pro Woche`));
  } else {
...
function renderMostAvailability(entries, prefix = "") {
  ...
  const slots = element("div", "common-slots");
  for (const { day, start, end } of best.slots) {
    const slot = element("div", "common-slot");
    slot.append(element("strong", "", dayLabels[day]), element("span", "", `${start}–${end} Uhr`),
      element("small", "availability-attendance", `${best.availableCount} von ${best.count}`));
    slots.append(slot);
  }
  const summary = best.availableCount === best.count
    ? `Zu diesen Zeiten können sogar alle ${best.count} bisherigen Teilnehmer.`
    : `Beste Überschneidung: ${best.availableCount} von ${best.count} bisherigen Teilnehmern. Die Gruppe kann je nach Zeitfenster unterschiedlich sein.`;
