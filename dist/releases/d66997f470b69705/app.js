...
function clearAdminData() {
  state.entries = [];
  for (const selector of ["#stats-grid", "#mode-stats", "#common-availability", "#most-availability", "#roster-list"]) $(selector).replaceChildren();
...
function renderPublicRoster() {
  ...
  for (const entry of rows) {
    ...
    if (group !== previousGroup) {
      ...
      // When the roster is grouped by role, the heading carries that role's artwork as well.
      if (sortBy === "role") {
    ...
    // No role badge on the card: the group heading above the cards already names the role, so a
    // second label would only repeat it. Sorting by class or spec shows no role heading, so the
    // role leads the expanded details instead.
    ...
    details.append(detail("Rolle", raidRoleLabel(entry.role), "public-raid-meta"));
...
function renderAvailabilityInsights(entries, prefix = "") {
  ...
  if (shared.status === "available") {
    ...
    const messages = { empty: "Noch keine Rückmeldungen vorhanden.", incomplete: "Die Verfügbarkeit ist noch nicht für alle vollständig angegeben.", none: "Es gibt keinen gemeinsamen Tag mit einem Zeitfenster, das bei allen passt." };
