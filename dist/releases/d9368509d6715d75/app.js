  ...
  for (const entry of rows) {
    ...
    if (group !== previousGroup) {
      ...
      if (groupIcon) {
        ...
        headIcon.src = iconUrl(groupIcon);
        headIcon.alt = "";
        heading.append(headIcon);
      }
      heading.append(document.createTextNode(raidRoleLabel(group)));
      if (sortBy === "spec" && entry.class_name) {
        heading.append(element("small", "public-group-note", displayChoice(entry.class_name)));
      }
      if (sortBy === "role") {
...
function renderAverageRaidDays(entries, prefix = "") {
  ...
  // Only the headline number, with the answers behind a hover: who wants how many raid days. The names are
  // coloured by role but not grouped by it - here the question is the number, not the role.
  const slot = element("div", "common-slot");
  slot.append(element("strong", "", averageText), element("span", "", "Tage im Schnitt"));

  const details = element("div", "slot-details");
  details.setAttribute("role", "tooltip");
  const kopfzeile = element("div", "slot-details-head");
  kopfzeile.append(
    element("strong", "slot-details-day", averageText),
    element("span", "slot-details-time", "Tage im Schnitt"));
  details.append(kopfzeile);

  const gruppe = element("div", "slot-details-group can");
  gruppe.append(element("span", "slot-details-label", "Maximale Raidtage je Person"));
  // The stored role is a summary for damage dealers - the database keeps "Damage" there - so the role is
  // derived the way the roster derives it. Without this every damage dealer would share one colour.
  const mitRolle = entries.map((entry) => ({ entry, role: raidRole(entry) }));
  const hoechste = Math.max(...values);
  for (let tage = hoechste; tage >= 1; tage -= 1) {
    const dieser = mitRolle.filter(({ entry }) => Number(entry.max_raid_days) === tage);
    if (!dieser.length) continue;
    // The same frame as the role blocks, without a role name in the heading.
    const block = element("div", "slot-role-block");
    ...
    // Inside a day the roles decide and the name inside that. No role label here: the names carry the
    // role colour, and a label above every day would only repeat itself.
  ...
  details.append(gruppe);
  slot.append(details);
  slot.tabIndex = 0;
  slot.setAttribute("aria-label", `Im Schnitt ${averageText} maximale Raidtage, ${values.length} von ${entries.length} Angaben`);
  // The script places the panel and, on a touch screen, opens and closes it; the card's :hover shows
  // it on a pointer, exactly as in the compromise card.
  wireSlotDetails(slot, details);

  const slots = element("div", "common-slots");
  slots.append(slot);
  target.append(slots);
  // The hint sits under the tile, the way the compromise card shows it.
  target.append(element("p", "common-caption", detailsHint()));
}
...
// The hover card lists both parts one heading per role, in the roster's own role order. The name decides
// within a role for the people who fit, and class with spec for the people who do not.
const byName = (a, b) => String(a.name ?? "").localeCompare(String(b.name ?? ""), "de");
const byClassThenSpec = (a, b) =>
  String(a.className ?? "").localeCompare(String(b.className ?? ""), "de") ||
  String(a.spec ?? "").localeCompare(String(b.spec ?? ""), "de") || byName(a, b);
const byRole = (leute, sortieren) => {
  ...
  for (const role of [...raidRoleOrder, ...rest]) {
    const dieser = leute.filter((person) => person.role === role).sort(sortieren);
...
// inside the window when it would run past an edge. The panel is capped to the viewport by the
// stylesheet, so the measurement below always describes a box that fits on the screen.
...
// A real pointer shows the panel on hover and the stylesheet does that on its own. A touch screen has
// no hover, so the tap decides: it opens the panel on the tile and a tap anywhere else - including on
// the open panel itself - closes it again. Otherwise the panel would cover the card for good.
const pointerHovers = () => matchMedia("(hover:hover)").matches;
const detailsHint = () => pointerHovers() ? "Mouseover für Details" : "Tippen für Details";

// The one tile a finger opened. A pointer never sets it, so hover and tap cannot get in each other's way.
let openSlot = null;

function closeSlotDetails() {
  if (openSlot) openSlot.classList.remove("slot-open");
  openSlot = null;
}

// Wires one tile and its panel. Shared by both availability cards so the two behave the same.
function wireSlotDetails(slot, details) {
  const zeigen = () => {
    details.scrollTop = 0;
    placeSlotDetails(slot, details);
  };
  slot.addEventListener("mouseenter", zeigen);
  slot.addEventListener("focusin", zeigen);
  slot.addEventListener("click", (event) => {
    if (pointerHovers()) return;
    // The open panel is part of the tile, so a tap on it would otherwise never close it again.
    if (event.target.closest(".slot-details") || openSlot === slot) {
      closeSlotDetails();
      return;
    }
    closeSlotDetails();
...
document.addEventListener("click", (event) => {
  if (pointerHovers()) return;
  if (!event.target.closest(".common-slot")) closeSlotDetails();
});
document.addEventListener("keydown", (event) => {
  if (event.key !== "Escape") return;
  closeSlotDetails();
  // Escape has to take the focus off the tile as well, otherwise :focus-visible would keep the panel up.
  const aktiv = document.activeElement;
  if (aktiv && aktiv.closest && aktiv.closest(".common-slot")) aktiv.blur();
});
// The panel is fixed in the window, so it has to follow the tile while the page moves. Once the tile
// has left the screen there is nothing left to explain and the panel closes.
addEventListener("scroll", () => {
  if (!openSlot || pointerHovers()) return;
  const kachel = openSlot.getBoundingClientRect();
  if (kachel.bottom < 0 || kachel.top > innerHeight) {
    closeSlotDetails();
    return;
  }
  placeSlotDetails(openSlot, openSlot.querySelector(".slot-details"));
}, { passive: true });

function renderMostAvailability(entries, prefix = "") {
  const target = $(`#${prefix}most-availability`);
  target.replaceChildren();
  // raidRole decides the role, so it is handed in rather than guessed from the stored field.
  const best = mostAvailability(entries, 5, raidRole, raidRoleOrder);
  ...
  for (const option of best.options) {
    const slot = element("div", "common-slot");
    const head = element("div", "common-slot-head");
    ...
    slot.append(roles);
    // The hover card the browser would not draw for us: everybody who fits, sorted by role, and
    // everybody who does not, sorted by class and spec. A title attribute cannot be laid out.
    const details = element("div", "slot-details");
    details.setAttribute("role", "tooltip");
    // The head names the window the way the tile below does - day, time, attendance - so the panel reads
    // as belonging to that tile.
    const kopfzeile = element("div", "slot-details-head");
    kopfzeile.append(
      element("strong", "slot-details-day", dayLabels[option.day]),
      element("span", "slot-details-time", `${option.start}–${option.end} Uhr`),
      element("small", "slot-details-count", `${option.count} von ${best.count}`));
    details.append(kopfzeile);
    ...
    const gruppen = [
      { titel: `Verfügbar (${option.canAttend.length})`, art: "can", bloecke: byRole(option.canAttend, byName) },
      // Both parts are grouped by role; here the class and spec decide the order inside a role.
      { titel: `Keine Zeit (${option.cannotAttend.length})`, art: "cannot",
        bloecke: byRole(option.cannotAttend, byClassThenSpec) },
    ];
    ...
    // The script places the panel and, on a touch screen, opens and closes it. On a pointer whether it
    // is shown stays the card's :hover, which cannot flicker: tracking enter and leave by hand does not
    // work here, because the card redraws its contents on hover and the leave arrives right after the
    // enter.
    wireSlotDetails(slot, details);
    slots.append(slot);
  }
