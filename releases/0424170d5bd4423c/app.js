...
function renderMostAvailability(entries, prefix = "") {
  ...
  for (const option of best.options) {
    ...
    slot.append(roles);
    // Who exactly, on hover: the names this window fits and the names it does not.
    slot.title = [
      `${dayLabels[option.day]} ${option.start}–${option.end}`,
      `Dabei (${option.canAttend.length}): ${option.canAttend.join(", ")}`,
      `Nicht dabei (${option.cannotAttend.length}): ${option.cannotAttend.join(", ") || "–"}`,
    ].join("\n");
    slot.tabIndex = 0;
