...
function renderAverageRaidDays(entries, prefix = "") {
  ...
  kopfzeile.append(
    ...
    element("small", "slot-details-count", `${values.length} von ${entries.length} Angaben`));
  ...
  for (let tage = hoechste; tage >= 1; tage -= 1) {
    const dieser = entries
      .filter((entry) => Number(entry.max_raid_days) === tage)
    ...
    for (const entry of dieser) {
      const zeile = element("span", `slot-person role-${raidRoleClass(raidRole(entry))}`);
