...
function renderMostAvailability(entries, prefix = "") {
  ...
  for (const option of best.options) {
    ...
    // Hover with a short grace period: the panel can reach over neighbouring cards, and without the
    // delay it would flicker away the moment the pointer crosses it.
    let verstecken = 0;
    const zeigen = () => {
      clearTimeout(verstecken);
      placeSlotDetails(slot, details);
    };
    const spaeterVerstecken = () => {
      clearTimeout(verstecken);
      verstecken = setTimeout(() => {
        details.style.visibility = "hidden";
        details.style.opacity = "0";
      }, 120);
    };
    slot.addEventListener("mouseenter", zeigen);
    slot.addEventListener("mouseleave", spaeterVerstecken);
    slot.addEventListener("focusin", zeigen);
    slot.addEventListener("focusout", spaeterVerstecken);
    // Moving onto the panel itself keeps it up, so it can be read without holding the pointer still.
    details.addEventListener("mouseenter", () => clearTimeout(verstecken));
    details.addEventListener("mouseleave", spaeterVerstecken);
    slots.append(slot);
  }
  target.append(slots);
