...
// Find the largest group offering time outside the all-participant intersection.
// Extend each candidate over its full overlap, including periods when everyone can join.
export function mostAvailability(entries) {
  ...
  for (const day of dayOrder) {
    ...
    for (let index = 0; index < boundaries.length - 1; index++) {
      ...
      const members = daily.filter((window) => window.start <= start && window.end >= end);
      if (!members.length || members.length === entries.length || members.length < result.availableCount) continue;
      if (members.length > result.availableCount) {
        result.availableCount = members.length;
        result.slots = [];
      }
      const slot = { day, start: clock(Math.max(...members.map((window) => window.start))),
        end: clock(Math.min(...members.map((window) => window.end))) };
      if (!result.slots.some((existing) => existing.day === day && existing.start === slot.start && existing.end === slot.end)) result.slots.push(slot);
    }
  }
