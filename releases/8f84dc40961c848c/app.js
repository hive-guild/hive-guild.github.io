...
function renderPublicRoster() {
  ...
  for (const role of raidRoleOrder) {
    ...
    const tile = element("div", `public-stat role-${raidRoleClass(role)}`);
    const icon = document.createElement("img");
  ...
  for (const entry of rows) {
    ...
    vision.append(element("small", "", "Raid-Vorstellung"), element("p", "", entry.raid_vision?.trim() || "Noch keine Angabe"));
    card.append(vision);
    list.append(card);
  }
}
