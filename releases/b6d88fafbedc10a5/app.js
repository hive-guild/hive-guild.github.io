...
const serverModes = [{ name: "PVE", icon: "server-pve.svg" }, { name: "PVP", icon: "server-pvp.svg" }];
...
function serverModeBadge(value) {
  ...
  badge.append(document.createTextNode(`Server: ${mode?.name || "Noch offen"}`));
  return badge;
}
...
function expandedInfo(entry) {
  ...
  for (const [label, value] of [
    ...
    ["Bevorzugter Server", entry.server_mode || "Noch offen"],
