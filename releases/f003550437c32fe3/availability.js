...
export function mostAvailability(entries, limit = 5, roleOf = (entry) => entry?.role, roleOrder = windowRoleOrder) {
  ...
  result.options = best.map((candidate) => ({
    ...
    // Who this window is for. The client turns the numbers into badges and the names into a tooltip -
    // there is nothing secret in here, the roster is public either way.
    roles: countRoles(candidate.members, windows, roleOf, roleOrder),
    canAttend: candidate.members.map((index) => entries[index]?.name).filter((name) => name != null),
    cannotAttend: windows.map((window) => window.index)
      .filter((index) => !candidate.members.includes(index))
      .map((index) => entries[index]?.name).filter((name) => name != null),
  }));
